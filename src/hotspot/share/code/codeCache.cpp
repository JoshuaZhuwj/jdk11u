...
void CodeCache::trace_non_profiled_hot_code_heap_activities(outputStream* st, const char* event, nmethod* nm, bool detailed) {
  ...
  CodeHeap* heap = CodeCache::get_code_heap(nm);
  assert(heap != NULL, "code heap of nm should not be null");
  ...
  if (strncmp(event, "Free", 4)) {
    st->print_cr("%s a hottest method in NonProfiledHotCodeHeap: %s::%s%s",
                 event, nm->method()->klass_name()->as_utf8(), nm->method()->name()->as_utf8(),
                 nm->method()->signature()->as_utf8());
  } else {
